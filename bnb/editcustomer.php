...
//save the customer data if the error flag is still clear and customer id is > 0
    if ($error == 0 and $id > 0) {
        $query = "UPDATE customers SET firstname=?,lastname=?,email=? WHERE customerID=?";
...
$query = 'SELECT customerID,firstname,lastname,email FROM customers WHERE customerID='.$id;
